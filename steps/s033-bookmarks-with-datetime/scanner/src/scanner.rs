...
impl BookmarkScanner {
    ...
    pub fn try_parse_dt_a(&mut self) -> Option<bookmark_item::Item> {
        ...
        self.next()?;
        let epoch_time: u32;
        match self.add_date.parse::<u32>() {
            Ok(value) => epoch_time = value,
            Err(_)        => return None,
        }
    ...
    fn parse_a(&mut self) -> Option<()> {
        ...
        if let BookmarkToken::StartToken{name: _, attributes} = &self.tokens[self.index] {
            ...
            let add_date = if let Some(add_date) = attributes.get("add_date") {
                add_date.clone()
            } else {
                "".to_string()
            };
    ...
    fn parse_h3(&mut self) -> Option<()> {
        ...
        if let BookmarkToken::StartToken{name: _, attributes} = &self.tokens[self.index] {
            self.add_date = if let Some(add_date) = attributes.get("add_date") {
                add_date.clone()
            } else {
                "".to_string()
            }
