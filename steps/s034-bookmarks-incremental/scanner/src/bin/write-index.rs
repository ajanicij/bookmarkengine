...
fn run() -> Result<(), Box<dyn Error>> {
    ...
    max_age = match max_age_str.parse::<u32>() {
        Ok(value) => Some(value),
        Err(err) => {
            None
        }
    ...
    if let Some(index_str) = args.index.as_path().to_str() {
        for bookmark in &scanner.bookmarks {
            if let Some(max_age_num) = max_age {
                if let Item::Bookmark{ description: _, path: _, href: _, last_modified: last_modified } = bookmark {
                    if days_from_epoch(*last_modified) > max_age_num {
                        // Too old; skip.
                        continue;
                    }
                }
