...
pub fn write_index(index_path: &str, bookmark: &Item) -> Result<(), Box<dyn Error>> {
    ...
    let result = client.get(url_string.clone()).send()?;
    // println!("{:?}", result);
...
pub fn search(index_path: &str, query_str: &str) -> Result<(), Box<dyn Error>> {
    ...
    let query_parser = QueryParser::for_index(&index, vec![title_field, body_field, description_field, path_field, url_field]);
