...
fn run() -> Result<String, Box<dyn Error>> {
    ...
    match args.command {
        Command::Write{index, bookmark, max_age} => {
            // let index_str = index.into_os_string().into_string().unwrap();

            println!("write(index={:?}, bookmark={:?}, max_age={:?}", index, bookmark, max_age);
            cmd_write(index, bookmark, max_age)?;
        },
...
fn cmd_write(index: PathBuf, bookmarks: PathBuf, max_age: String) ->
Result<String, Box<dyn Error>> {
    ...
    let mut indexer = utils::Indexer::new(&index_str)?;

    // Process bookmarks.
    for bookmark in &scanner.bookmarks {
        // println!("{:?}", bookmark);

        let _ = indexer.write(bookmark);
    }
