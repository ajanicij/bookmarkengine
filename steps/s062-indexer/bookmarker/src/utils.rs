...
impl Indexer {
    ...
    pub fn write(&self, message: BookmarkMessage)  -> Result<(), Box<dyn Error>> {
        ...
        match bookmark {
            Item::Bookmark{ description: description_field, path: path_field, href: _href, last_modified: _ } => {
                // url_string = href.to_string();
                path_string = path_field.to_string();
                description_string = description_field.to_string();
            },
    ...
    pub fn index(&mut self, bookmarks: Vec<bookmark_item::Item>,
        commit_period: u32) -> Result<(), Box<dyn Error>> {
